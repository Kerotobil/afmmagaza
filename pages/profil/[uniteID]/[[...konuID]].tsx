        ...
        <div>
            ...
            <div className="flex min-h-screen">
                ...
                <div className="ml-8 grid grid-cols-3 gap-16">
                    {
                        data && (konuID == null) && data[0].Ders.map(item => <MyLessons urlProxy={`/profil/${uniteID}/${item.id}`} DersAdi={item.DersAdi} complete={item.complete}  path={`/profil/[uniteID]/[[...konuID]]`} />)
                    }
                    {
                        data && (konuID != null) && data[0].Ders.map((element) => 
                                (element.id == konuID) &&  element.icerik.map(item => 
                                <MyLessons urlProxy={`/video/${konuID}/${item.id}/0`} DersAdi={item.DersAdi} complete={item.complete} path={`/video/[uniteID]/[konuID]/[icerikID]`} />) )
                    }
